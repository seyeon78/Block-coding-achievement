...
# 프로젝트 별 구현성 계산
def calculate_project_scores_feasibility(input_data, output_project_file):
    ...
    output_project_data = pd.DataFrame({
        'userid': df['userid'],
    ...
    output_project_data.to_csv(output_project_file, index=False)
    return output_project_data

# 사용자 별 최종 구현성 계산
def calculate_user_score_feasibility(output_project_file, output_user_file):
    df = pd.read_csv(output_project_file)
    ...
    # 최종 데이터프레임 생성 및 저장
    final_values = user_feasibility_scores[['userid', 'feasibilityScore', 'feasibilityRate']].copy()
    final_values.to_csv(output_user_file, index=False)
    return final_values

# 메인 함수
def main_feasibility():
    input_data = 'filtered_log.csv'
    output_project_file = 'feasibility_project.csv'
    output_user_file = 'feasibility_user.csv'
    calculate_project_scores_feasibility(input_data, output_project_file)
    calculate_user_score_feasibility(output_project_file, output_user_file)
